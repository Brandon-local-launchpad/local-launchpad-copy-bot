...
**Before writing the final version, you must count.** Draft the full sentence with all required elements (primary category, target location, two service references, CTA with phone number), then count the resolved character length in your reasoning, the same way you do for the title tag. State the number.
...
3. Drop the phone number from the meta description itself, ending on a CTA that doesn't need it (e.g. "Book your free visit today") — this overrides the "referencing the phone number placeholder" instruction above when length forces a choice between the two.
