...
**Before writing the final version, you must count.** Draft the full sentence with all required elements (target service, primary area, category reference, CTA with phone number), then count the resolved character length in your reasoning, the same way you do for the title tag. State the number.
...
1. Drop the PARENT CATEGORY reference.
